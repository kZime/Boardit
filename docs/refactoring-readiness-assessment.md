...
| 维度 | 支持程度 | 结论 |
|---|---|---|
| UI 组件 | 🔧 部分支持 | 编辑器和发布 UI 已存在，但状态与行为集中在 Editor |
| 数据获取 | 🔧 部分支持 | React Query、OpenAPI、Orval 可复用，但调用方式不统一 |
| 状态管理 | ⚠️ 存在风险 | 服务端缓存、编辑会话和界面状态缺少明确边界 |
| 权限控制 | ⚠️ 存在风险 | JWT 基础存在，但资源作用域和 refresh 会话边界不完整 |
| 路由 | ✅ 已支持 | 路由规模简单，可保持现状并按模块注册 |
| 类型定义 | 🔧 部分支持 | OpenAPI 生成类型是优势，但手写 DTO 和 `gin.H` 造成漂移 |
| 数据层 | ⚠️ 存在风险 | GORM 和 PostgreSQL 可保留，全局 DB、AutoMigrate 和事务边界需调整 |
| 自动化测试 | 🔧 部分支持 | 后端基础测试可复用，核心领域与前端测试保护不足 |
...
- [ ] 纯结构重构 PR 不改变现有 API 契约和用户可见行为。
- [ ] 每个已知 bug 修复均具有独立回归测试。
- [ ] 前后端 lint、类型检查、构建和测试持续通过。
...
- [ ] 资源查询默认强制用户作用域，跨用户测试覆盖 notes、folders、revisions 和 search。
- [ ] 树重排和笔记更新/版本创建具有原子事务。
- [ ] 数据库变更有版本号、向前迁移和回滚说明。
- [ ] refresh token 可被识别、轮换和撤销，不与 access token 混用。
